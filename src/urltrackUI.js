...
import { end_point, url_eater } from "./constants";
import { makeReq } from "./appwrite_mod";
...
async function urlpostData(data,setData)
{
  //const xurl = "/";
  //const xurl = url_eater;
  ...
  try {
    const response = await makeReq(data["content"]);
    //fetch(url, requestOptions);

    const xresponseData = response.responseData;
    //await response.json();
    const responseData = {"loader_on":false,"show_ans":true,"content":xresponseData["content"]};
    await xpostData(responseData,setData);
    //setData(false);
    //setData(responseData); // Store response data in state or do something with it
  } catch (error) {
    console.error('Error:', error);
...
function SaveURLBtn(btn_props) {
   ...
   const smBtnClicked = ()=>{
    ...
    changeSMClick(!is_SM_clicked);
      ...
      urlpostData(content,changeSMClick);
